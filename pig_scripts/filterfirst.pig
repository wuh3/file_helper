...
-- Remove unnecessary records (Cancelled or Diverted flights) and filter by date range
Flights1_Filtered = FILTER Flights1 BY
    Cancelled1 == 0 AND Diverted1 == 0 AND
    ((Year1 == 2007 AND Month1 >= 6) OR (Year1 == 2008 AND Month1 <= 5));

Flights2_Filtered = FILTER Flights2 BY
    Cancelled2 == 0 AND Diverted2 == 0 AND
    ((Year2 == 2007 AND Month2 >= 6) OR (Year2 == 2008 AND Month2 <= 5));

-- Project necessary fields
Flights1_Clean = FOREACH Flights1_Filtered GENERATE
    FlightDate1, Dest1 AS JoinAirport, ArrTime1, ArrDelayMinutes1;

Flights2_Clean = FOREACH Flights2_Filtered GENERATE
    FlightDate2, Origin2 AS JoinAirport, DepTime2, DepDelayMinutes2;
